interface RelievingLetterTemplateData {
  companyName: string;
  logoUrl?: string | null;
...
export const relievingLetterTemplate = (
  data: RelievingLetterTemplateData
): string => {
  ...
  const resignationDate = escapeHtml(data.resignationDate);
  const lastWorkingDay = escapeHtml(data.lastWorkingDay);

  const documentNumber = escapeHtml(
    data.documentNumber
  );
    ...
    <div class="content-wrapper">
      ...
      <div class="body">
        ...
        <p>
          ...
          ${employeeName}
          has been relieved from the services of
          ${companyName} with effect from
          <strong>${lastWorkingDay}</strong>.
        </p>

        <p>
          The employee's resignation was accepted by the
          organization and all applicable exit formalities
          and departmental clearances have been completed.
        </p>

        <p>
          We thank ${employeeName} for the services
          rendered during the tenure of employment and
          wish them success in all future endeavors.
        </p>
